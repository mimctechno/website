import { Terminal, Zap, Shield, Globe, BarChart3, Database, ChevronRight, ArrowRight } from 'lucide-react';
...
const services = [
  {
    ...
    bullets: ['Custom module development', 'Legacy system migration', 'Cloud & on-premise deployments', 'Real-time reporting dashboards'],
  ...
  },
  {
    ...
    bullets: ['Official Meta API partner setup', 'Bulk messaging automation', 'Chatbot & auto-reply systems', 'CRM & ERP integration'],
    accent: '#25D366',
  },
  {
    ...
    bullets: ['Zero manual data entry', 'Instant invoice delivery', 'Payment reminder automation', 'Tally ERP 9 & Tally Prime'],
  ...
  },
  {
    ...
    bullets: ['Custom UI/UX design', 'Technical SEO & Core Web Vitals', 'Local & international SEO', 'Performance optimization'],
  ...
  },
  {
    ...
    bullets: ['Search engine marketing (SEM)', 'Content & keyword strategy', 'Google Analytics & reporting', 'Lead funnel optimization'],
    accent: 'var(--color-cyber-accent)',
  },
  {
    ...
    bullets: ['Technology stack audits', 'Digital transformation roadmaps', 'Vendor selection & negotiation', 'Implementation oversight'],
    accent: 'var(--color-cyber-accent2)',
  },
];
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-end">
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-black uppercase tracking-widest font-[var(--font-cyber-head)] leading-none text-white">
              <span className="block cyber-glitch" data-text="OUR">OUR</span>
              <span className="bg-gradient-to-r from-[var(--color-cyber-accent)] via-[var(--color-cyber-accent3)] to-[var(--color-cyber-accent2)] bg-clip-text text-transparent">SERVICES</span>
            </h1>
            <p className="text-[var(--color-cyber-muted-fg)] font-[var(--font-cyber-accent)] uppercase tracking-wider text-sm leading-relaxed border-l-2 border-[var(--color-cyber-accent)] pl-4">
              From enterprise ERP deployments to WhatsApp API automation — we engineer the infrastructure that scales your business. Each service is delivered by specialists, not generalists.
            </p>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {services.map((s) => (
              <Link
                ...
                style={{ background: `linear-gradient(135deg, ${s.accent}33, transparent)` }}
              >
                <div className="bg-[var(--color-cyber-card)] cyber-chamfer p-8 h-full flex flex-col relative overflow-hidden transition-all duration-300 group-hover:bg-[#0a0a0f]">
                  ...
                  <div className="mb-6 self-start">
                    <span className="text-[10px] font-[var(--font-cyber-accent)] uppercase tracking-[0.3em] px-3 py-1 border"
                      style={{ color: s.accent, borderColor: `${s.accent}44`, background: `${s.accent}11` }}>
                      {s.badge}
                    </span>
                  </div>

                  {/* Icon */}
                  <div className="w-14 h-14 border border-[var(--color-cyber-border)] flex items-center justify-center mb-6 bg-black transition-all group-hover:border-current"
                    style={{ ['--tw-shadow-color' as string]: s.accent }}>
                    <s.icon className="w-7 h-7 transition-colors" strokeWidth={1.5}
                      style={{ color: 'var(--color-cyber-muted-fg)' }}
                    />
                  </div>

                  <h2 className="text-lg md:text-xl font-bold font-[var(--font-cyber-head)] uppercase tracking-widest mb-2 text-white transition-colors"
                    style={{}}>
                    {s.title}
                  </h2>
                  <p className="font-[var(--font-cyber-accent)] text-xs uppercase tracking-widest mb-4"
                  ...
                  <ul className="space-y-2 mb-8">
                    {s.bullets.map(b => (
                      <li key={b} className="flex items-center gap-2 text-xs font-[var(--font-cyber-accent)] uppercase tracking-wider text-[var(--color-cyber-muted-fg)]">
                  ...
                  {/* CTA */}
                  <div className="flex items-center gap-2 font-[var(--font-cyber-accent)] text-xs uppercase tracking-widest font-bold transition-all"
                    style={{ color: s.accent }}>
        ...
        <section className="mb-24 border border-[var(--color-cyber-border)] p-10 md:p-16 relative overflow-hidden">
          ...
          <h2 className="text-3xl md:text-4xl font-bold font-[var(--font-cyber-head)] uppercase tracking-widest text-white mb-16 flex items-center gap-4">
            <span className="text-[var(--color-cyber-accent)]">&gt;&gt;</span> HOW_WE_WORK
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              { step: '01', title: 'Discovery', desc: 'We audit your current systems and map exact requirements.' },
              { step: '02', title: 'Blueprint', desc: 'We engineer a detailed solution architecture for your approval.' },
              { step: '03', title: 'Deploy', desc: 'Our team builds and deploys — fast, clean, with zero disruption.' },
              { step: '04', title: 'Support', desc: 'Ongoing monitoring, updates, and a dedicated account manager.' },
            ].map((p, i) => (
              <div key={p.step} className="relative">
                ...
                <div className="font-[var(--font-cyber-head)] text-5xl font-black text-[var(--color-cyber-accent)]/20 mb-4">{p.step}</div>
                <h3 className="font-[var(--font-cyber-head)] text-lg font-bold uppercase tracking-widest text-white mb-3">{p.title}</h3>
        ...
        <section className="text-center border border-[var(--color-cyber-accent)] p-12 relative overflow-hidden">
          ...
          <h2 className="text-4xl font-black uppercase tracking-widest font-[var(--font-cyber-head)] text-white mb-6 relative z-10">
            NOT SURE WHICH <span className="text-[var(--color-cyber-accent)]">SERVICE YOU NEED?</span>
          </h2>
          <p className="text-[var(--color-cyber-muted-fg)] font-[var(--font-cyber-accent)] uppercase tracking-widest text-sm mb-10 relative z-10">
            Book a free 30-minute discovery call. No sales pitch. Just honest advice.
          </p>
          <Link to="/contact"
