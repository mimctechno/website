import { BarChart3, CheckCircle, ChevronRight, Target, TrendingUp, Search, Globe, Megaphone, Mail } from 'lucide-react';
import { Link } from 'react-router-dom';
import Layout from '../../components/Layout';

const features = [
  { icon: Search, title: 'Search Engine Marketing', desc: 'Google Ads campaigns engineered for maximum ROI. Keyword targeting, ad copy, bid strategy, and landing page optimisation.' },
  { icon: TrendingUp, title: 'SEO & Organic Growth', desc: 'Long-term organic traffic growth through technical SEO, content strategy, link building, and keyword domination.' },
  { icon: Globe, title: 'Local SEO', desc: 'Dominate "near me" searches. Google Business Profile, local citations, and region-specific landing pages for your target cities.' },
  { icon: Megaphone, title: 'Social Media Marketing', desc: 'LinkedIn, Instagram, and Facebook campaigns targeting your exact buyer persona — with creative content that converts.' },
  { icon: Mail, title: 'Email & WhatsApp Marketing', desc: 'Automated nurture sequences via email and WhatsApp. Keep leads warm, re-engage past clients, and drive repeat business.' },
  { icon: Target, title: 'Conversion Rate Optimisation', desc: 'A/B testing, heatmaps, and funnel analysis to squeeze more leads out of your existing traffic — without increasing ad spend.' },
];

const channels = [
  { name: 'Google Search Ads', desc: 'Capture high-intent buyers at the exact moment they search for your service.' },
  { name: 'Google SEO', desc: 'Build a permanent source of organic leads that doesn\'t cost per click.' },
  { name: 'LinkedIn Ads', desc: 'B2B targeting by job title, company size, and industry for enterprise deals.' },
  { name: 'Meta (Facebook/Instagram)', desc: 'Brand awareness and retargeting campaigns for B2C and SME audiences.' },
  { name: 'WhatsApp Campaigns', desc: 'Direct outreach via WhatsApp API — 98% open rates vs 22% for email.' },
  { name: 'Content Marketing', desc: 'Blog posts, case studies, and guides that rank on Google and build authority.' },
];

const faqs = [
  { q: 'How long does SEO take to show results?', a: 'SEO is a long-term investment. Most clients see measurable improvements in rankings within 3–6 months, and significant organic traffic growth within 6–12 months. The timeline depends on competition in your niche, your current domain authority, and the quality of content we produce. We provide monthly reports so you can track progress.' },
  { q: 'What is the minimum budget for Google Ads?', a: 'We recommend a minimum ad spend of $500–$1,000/month to gather meaningful data and optimise effectively. Our management fee is separate. Lower budgets are possible for highly targeted local campaigns, but scale limits results. We\'ll advise the right budget for your goals on a discovery call.' },
  ...
  { q: 'How do you measure marketing performance?', a: 'We track the metrics that matter to your business: leads generated, cost per lead, keyword rankings, organic traffic growth, and conversion rate. You receive a comprehensive monthly report with clear data and our interpretation of what it means for your growth.' },
  { q: 'Can you help with local SEO for multiple cities?', a: 'Yes. We create city-specific landing pages, localise your Google Business Profile, build local citations, and develop region-specific content. This is especially effective for businesses targeting multiple cities in India, Brazil, or other high-growth markets.' },
];
...
const serviceSchema = {
  ...
  description: 'SEO, Google Ads, social media marketing, WhatsApp campaigns, and conversion optimisation services by MIMC Technologies.',
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <section className="mb-24 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            ...
            <h1 className="text-5xl md:text-6xl font-black uppercase tracking-widest font-[var(--font-cyber-head)] leading-none text-white">
              <span className="block cyber-glitch" data-text="DIGITAL">DIGITAL</span>
              <span className="bg-gradient-to-r from-[var(--color-cyber-accent)] to-[var(--color-cyber-accent3)] bg-clip-text text-transparent">MARKETING</span>
              <span className="block text-white text-3xl md:text-4xl mt-2">THAT DRIVES LEADS</span>
            </h1>
            <p className="border-l-2 border-[var(--color-cyber-accent)] pl-4 font-[var(--font-cyber-accent)] text-[var(--color-cyber-muted-fg)] leading-relaxed uppercase tracking-wider text-sm">
              &gt; SEO, Google Ads, and WhatsApp campaigns.<br />
              &gt; Targeting India, Brazil, SEA &amp; LATAM markets.<br />
              &gt; Every rupee and dollar tracked and justified.<span className="inline-block w-2 h-4 bg-[var(--color-cyber-accent)] animate-blink ml-1 align-middle" />
            </p>
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              ...
              <Link to="/services" className="group inline-flex items-center justify-center gap-2 font-[var(--font-cyber-accent)] uppercase tracking-widest cyber-chamfer border-2 border-[var(--color-cyber-accent2)] text-[var(--color-cyber-accent2)] hover:bg-[var(--color-cyber-accent2)] hover:text-black transition-all duration-300 px-8 py-4">
                ALL SERVICES
              </Link>
            </div>
          </div>
            ...
            <div className="absolute inset-[2px] bg-[var(--color-cyber-card)] flex flex-col items-center justify-center gap-4 relative overflow-hidden">
              ...
              <div className="absolute bottom-8 left-8 right-8 flex items-end gap-2 opacity-15">
                {[40, 55, 35, 70, 60, 85, 75, 90].map((h, i) => (
                  <div key={i} className="flex-1 bg-[var(--color-cyber-accent)]" style={{ height: `${h}%` }} />
                ))}
              </div>
              <BarChart3 className="w-16 h-16 text-[var(--color-cyber-accent)] opacity-20 relative z-10" strokeWidth={1} />
              <p className="font-[var(--font-cyber-accent)] text-[var(--color-cyber-muted-fg)] text-xs uppercase tracking-widest text-center px-8 relative z-10">
                [IMAGE PLACEHOLDER]<br />Analytics dashboard / campaign results
              </p>
              <div className="absolute top-4 left-4 text-[10px] font-[var(--font-cyber-accent)] text-[var(--color-cyber-accent)] uppercase tracking-widest opacity-60">ANALYTICS // LIVE</div>
            </div>
          </div>
        </section>
        ...
        <section className="mb-24">
          <div className="flex items-center gap-4 mb-12 border-b border-[var(--color-cyber-border)] pb-4">
            <span className="text-[var(--color-cyber-accent)] font-[var(--font-cyber-accent)] text-sm uppercase tracking-widest">&gt;&gt; SERVICE_MODULES</span>
          </div>
        ...
        <section className="mb-24 border border-[var(--color-cyber-border)] p-10 md:p-14 relative overflow-hidden">
          ...
          <h2 className="text-2xl md:text-3xl font-bold font-[var(--font-cyber-head)] uppercase tracking-widest text-white mb-10 flex items-center gap-4">
            <span className="text-[var(--color-cyber-accent)]">&gt;&gt;</span> CHANNELS_WE_OPERATE
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {channels.map(c => (
              <div key={c.name} className="border border-[var(--color-cyber-border)] p-6 hover:border-[var(--color-cyber-accent)]/50 transition-colors group">
                <div className="flex items-center gap-2 mb-3">
                  <CheckCircle className="w-4 h-4 text-[var(--color-cyber-accent)] flex-shrink-0" />
                  <h3 className="font-[var(--font-cyber-head)] text-sm font-bold uppercase tracking-widest text-white group-hover:text-[var(--color-cyber-accent)] transition-colors">{c.name}</h3>
                </div>
                <p className="text-[var(--color-cyber-muted-fg)] text-xs uppercase tracking-wider leading-relaxed">{c.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-24 h-52 border border-[var(--color-cyber-border)] bg-[var(--color-cyber-card)] flex flex-col items-center justify-center gap-3 relative overflow-hidden">
          ...
          <TrendingUp className="w-10 h-10 text-[var(--color-cyber-accent)] opacity-30" strokeWidth={1} />
          <p className="font-[var(--font-cyber-accent)] text-[var(--color-cyber-muted-fg)] text-xs uppercase tracking-widest text-center">
            [IMAGE PLACEHOLDER]<br />Client campaign results / case study graphic
          </p>
        </section>
        ...
        <section className="text-center border-2 border-[var(--color-cyber-accent)] p-12 md:p-20 relative overflow-hidden">
          ...
          <h2 className="text-4xl md:text-5xl font-black uppercase tracking-widest font-[var(--font-cyber-head)] text-white mb-6 relative z-10">
            STOP HOPING.<br /><span className="text-[var(--color-cyber-accent)]">START RANKING.</span>
          </h2>
          <p className="font-[var(--font-cyber-accent)] text-[var(--color-cyber-muted-fg)] uppercase tracking-widest text-sm mb-10 relative z-10">
            Free 30-minute marketing audit. We'll show you exactly where your leads are going.
          </p>
          <Link to="/contact" className="group inline-flex items-center gap-3 font-[var(--font-cyber-accent)] uppercase tracking-widest cyber-chamfer border-2 border-[var(--color-cyber-accent)] bg-[var(--color-cyber-accent)] text-black hover:bg-transparent hover:text-[var(--color-cyber-accent)] transition-all duration-300 px-10 py-5 text-base font-bold relative z-10">
            BOOK FREE AUDIT <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </Link>
        </section>
